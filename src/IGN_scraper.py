...
def parse_page(outfile, page_contents, score_flag, verbose,
	outfile_lock, stderr_lock, print_lock):
...
def open_url_and_parse(outfile, curr_url, score_flag, force_max, verbose,
	outfile_lock, urlopen_lock, stderr_lock, print_lock, num_workers):
	...
	while run < force_max:
		...
		# Catch errors and attempt to reopen the url, if possible
		...
		# Try to reopen the url, if possible
	...
	# We succeeded in opening the file. Send it to the parser.
	parse_page(outfile, page_contents, score_flag, verbose,
		outfile_lock, stderr_lock, print_lock)

	# This process has now finished its work.
	num_workers.release()

# For each page in the index, spawn a process to open the page,
...
def main():
	...
	if (args.startindex < 0) or (args.startindex % INDEX_INCREMENT != 0) \
		...
		return 1
	# Vet the end index. As of August 24, 2013, the maximum end index
	# is 75450
